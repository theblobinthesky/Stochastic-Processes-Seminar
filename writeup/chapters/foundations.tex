...
\begin{proof}
    \begin{enumerate}
        ...
        \item The definition of mutual information yields:
            \begin{align*}
                I(X; Y) &= D(\prob[X = x, Y = y] \| \prob[X = x]\prob[Y = y])       && \text{(by def.~of mutual info.)} \\
                        &= D(\prob[Y = y, X = x] \| \prob[Y = y]\prob[X = x]) = I(Y; X)
            \end{align*}
...
\begin{definition}\label{def:conditional-entropy-mutual}
    ...
    \begin{align*}
        ...
        &= \mathbb{E}_{p}\paren{\log \frac{p_{Y \mid X}(Y \mid X)}{q_{Y \mid X}(Y \mid X)}} \\
