...
\begin{definition}
    Let $(\Omega, \mathcal{A}, \prob)$ be a probability space, let $\mathcal{X}$ be a countable set and
    let $X: \Omega \rightarrow \mathcal{X}$ be a discrete random variable on $(\Omega, \mathcal{A}, \prob)$.
    We can then define
    \begin{align*}
...
\begin{remark}
    ...
    \item
        ...
        be discrete random variables on $(\Omega, \mathcal{A}, \prob)$.
        From now on, the random variables $X, Y$ are always available for use.

    \item
        The shorthand notations $p(x) = \prob[X = x]$ and $p(y) = \prob[Y = y]$
        from \cite{Thomas:ElementsOfInformationTheory} exist.
        We do \textbf{not} use this bare shorthand in this paper.
        Instead, we use probability mass function notation
        $p_X(x) = \prob[X = x]$, $p_Y(y) = \prob[Y = y]$,
        and $p_{X, Y}(x, y) = \prob[X = x, Y = y]$.
        For conditional probabilities, we write
        $p_{Y \mid X}(y \mid x) = \prob[Y = y \mid X = x]$ (for $\prob[X = x] > 0$).

    \item
        For expectation with respect to a specific probability measure
        (equivalently, in a discrete setting, its probability mass function),
        we use them as subscripts on $\mathbb{E}$,
        for example $\mathbb{E}_{p_X}$, $\mathbb{E}_{p_{X, Y}}$, or more generally $\mathbb{E}_{p}$.
        If no subscript is given, expectation is taken with respect to the ambient measure $\prob$.
...
\begin{remark}[Existence of Entropy]
    Note that if $|\mathcal{X}|$ is finite, $(\forall b \in \mathbb{R}_+: H_b(X) \; \text{finite})$
    and $H(X) \le \log |\mathcal{X}|$ (see Theorem \ref{theorem:more-entropy-observations}).
...
\begin{definition}\label{def:entropy-mutual-info-def}
    ...
    \intertext{\noindent Let $p$ and $q$ be two probability mass functions on the same set $\mathcal{Z}$ and let $X \sim p$. We define the following:}
        \textbf{Relative Entropy:}    & \quad D(p \| q) = \mathbb{E}_{p}\paren{\log \frac{p(X)}{q(X)}} 
...
\begin{remark} \label{remark:relative-entropy-conventions}
    We have
    \begin{align*}
        D(p \| q)   &= \mathbb{E}_{p}\paren{\log \frac{p(X)}{q(X)}}                      && \text{(def.~of relative entropy)} \\
...
\begin{example}
    ...
    \begin{align*}
        ...
        \alpha= 0.5, \beta= 0.5: & \quad D(p_X \| p_Y) = \sum_{x = 0}^{20}{p_X(x) \log 1} = 0 \\
    \end{align*}
...
\begin{proof}
    \begin{align*}
        H(X, Y) &= -\mathbb{E}\paren{\log p_{X,Y}(X, Y)} 
        = -\mathbb{E}\paren{\log p_{Y \mid X}(Y \mid X)p_X(X)} \\
        &= -\mathbb{E}\paren{\log p_{Y \mid X}(Y \mid X)} - \mathbb{E}\paren{\log p_X(X)}
        = H(X) + H(Y \mid X)
    \end{align*}
\end{proof}
...
\begin{theorem}
    ...
    \begin{enumerate}
        \item $I(X; Y) = H(Y) - H(Y \mid X)$
        \item $I(X; Y) = I(Y; X)$
        \item $I(Y; X) = H(X) - H(X \mid Y)$
...
\begin{definition}\label{def:conditional-entropy-mutual}
    ...
    \begin{align*}
        ...
        &= \mathbb{E}_{p_X} D(p_{Y \mid X}(. \mid X) \| q_{Y \mid X}(. \mid X)) \\
        &= \sum_{x \in \mathcal{X}}{p_X(x) D(p_{Y \mid X}(. \mid x) \| q_{Y \mid X}(. \mid x))} \\
	    &= \mathbb{E}_{p_{X,Y}}\paren{\log \frac{p_{Y \mid X}(Y \mid X)}{q_{Y \mid X}(Y \mid X)}} \\
        &= \sum_{(x, y) \injoint}{p_{X,Y}(x, y) \log \frac{p_{Y \mid X}(y \mid x)}{q_{Y \mid X}(y \mid x)}} \\
...
\begin{example}
    ...
    These models are probability mass functions on $\mathcal{X} \times \mathcal{Y}$,
    with associated conditional probability mass functions $p_{Y \mid X}$ and $q_{Y \mid X}$. 
    We have lived on earth our entire lives. Our model of reality $p$ is correct! 
    We define it as
    \[
        p_{Y \mid X}(1 \mid x) = \begin{cases}
    ...
    We define it as 
    \[
        q_{Y \mid X}(1 \mid x) = \begin{cases}
    ...
    \begin{enumerate}
        \item $D(p_{Y \mid X}(\cdot \mid \textbf{NOV.}) \| q_{Y \mid X}(\cdot \mid \textbf{NOV.})) = 0 + 1 \log \frac{1}{0.5} = 1$
        \item $D(p_{Y \mid X}(\cdot \mid \textbf{DEC.}) \| q_{Y \mid X}(\cdot \mid \textbf{DEC.})) = 0 + 1 \log \frac{1}{0.5} = 1$
        \item $D(p_{Y \mid X}(\cdot \mid \textbf{JAN.}) \| q_{Y \mid X}(\cdot \mid \textbf{JAN.})) = 1 \log \frac{1}{0.5} + 0 = 1$
        \item Otherwise: $D(p_{Y \mid X}(\cdot \mid x) \| q_{Y \mid X}(\cdot \mid x)) = 0$
    \end{enumerate}
    We can now calculate the Conditional Relative Entropy:
    \begin{align*}
        D(p_{Y \mid X} \| q_{Y \mid X}) &= \sum_{x \in \mathcal{X}}{p_X(x) D(p_{Y \mid X}(\cdot \mid x) \| q_{Y \mid X}(\cdot \mid x))} \\
...
\begin{theorem}[Chain Rules] \label{theorem:chain-rules}
    ...
    \begin{enumerate}
        \item \label{item:chain-rules-entropy}
            \[
                H(X_1, \cdots, X_n) = \sum_{i=1}^{n}{H(X_i \mid X_{i-1}, \cdots, X_1)}
            \]
        \item
            \[
                D(p_{X,Y} \| q_{X,Y}) = D(p_X \| q_X) + D(p_{Y \mid X} \| q_{Y \mid X})
            \]
        \item
            \[
...
\begin{proof}
    \begin{enumerate}
        \item 
            ...
            \begin{align*}
                H(X_1, \cdots, X_n) &= H(X_n \mid X_1, \cdots, X_{n-1}) + H(X_1, \cdots, X_{n-1})          && \text{(apply base case)} \\
                &= H(X_n \mid X_1, \cdots, X_{n-1}) + \sum_{i=1}^{n-1}{H(X_i \mid X_{i-1}, \cdots, X_1)}   && \text{(induction hypothesis)}
            \end{align*}

        \item
            We prove the chain rule for relative entropy using a chain of equalities:
            \begin{align*}
                D(p_{X,Y} \| q_{X,Y})
                    &= \sum_{(x, y) \injoint}{p_{X,Y}(x, y) \log \frac{p_{X,Y}(x, y)}{q_{X,Y}(x, y)}} \\
                    &= \sum_{(x, y) \injoint}{p_{X,Y}(x, y) \log \frac{p_{Y \mid X}(y \mid x)p_X(x)}{q_{Y \mid X}(y \mid x)q_X(x)}} \\
                    &= \sum_{(x, y) \injoint}{p_{X,Y}(x, y) \log \frac{p_{Y \mid X}(y \mid x)}{q_{Y \mid X}(y \mid x)}}
                     + \sum_{(x, y) \injoint}{p_{X,Y}(x, y) \log \frac{p_X(x)}{q_X(x)}} \\
                    &= D(p_{Y \mid X} \| q_{Y \mid X}) + D(p_X \| q_X)
            \end{align*}
        
        \item
            We prove the chain rule for mutual information using a chain of equalities:
            \begin{align*}
                I(X_1, \cdots, X_n; Y) &= H(X_1, \cdots, X_n) - H(X_1, \cdots, X_n \mid Y) \\
                &= \sum_{i=1}^{n}{H(X_i \mid X_{i-1}, \cdots, X_1)} - \sum_{i=1}^{n}{H(X_i \mid X_{i-1}, \cdots, X_1, Y)} && \text{(using item \ref{item:chain-rules-entropy})} \\
                &= \sum_{i=1}^{n}{I(X_i; Y \mid X_{i-1}, \cdots, X_1)}                                                                  && \text{(using item \ref{item:chain-rules-entropy})}
            \end{align*}
