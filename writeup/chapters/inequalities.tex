...
\begin{proof}
    \begin{enumerate}
        \item
            ...
            \end{align*}
            Else: If $\mathcal{X}$ is countably infinite, the argument requires an additional approximation step.
            We omit this here and refer to \textit{Elements of Information Theory} \cite{Thomas:ElementsOfInformationTheory}.
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
		    ...
		    Lastly, the book Elements of Information Theory \cite{Thomas:ElementsOfInformationTheory} outlines the use of Jensen's Inequality to prove that entropy is maximized only by the uniform distribution. 
            Alternatively, it is more simple to use the theorem about Convexity of Entropy \ref{theorem:convexity-of-entropy} (it comes later and is based on the Log-Sum Inequality), 
            which confirms that $D(p_X \| q) = 0 \iff p_X = q$.
        \item
...
\begin{theorem}[Log-Sum Inequality]
    ...
    \]
    with equality iff. $\exists c > 0, \forall i: a_i = c b_i$
\end{theorem}
\begin{proof}
...
\begin{theorem}[Convexity of Entropy] \label{theorem:convexity-of-entropy}
	...
	\item
	    Zero relative entropy implies equality: $D(p \| q) = 0 \iff p = q$
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
            Given $p = q$, since $\log 1 = 0$, we have $D(p \| q) = 0$.
            Given $D(p \| q) = 0$, we have
            \begin{align*}
                0 &= D(p \| q) = \sum_{x \in \mathcal{X}}{p(x) \log \frac{p(x)}{q(x)}} \\
                &\ge \paren{\sum_{x \in \mathcal{X}}{p(x)}} \log \frac{\sum_{x \in \mathcal{X}}{p(x)}}{\sum_{x \in \mathcal{X}}{q(x)}} 
                = \log \frac{\sum_{x \in \mathcal{X}}{p(x)}}{\sum_{x \in \mathcal{X}}{q(x)}} 
                = \log 1 = 0
            \end{align*}
            So the Log-Sum Inequality is sharp and $p \ge 0, q \ge 0$, which implies $p = c q$ with $c > 0$.
            Since both $p$ and $q$ are probability mass functions and sum to $1$, $c = 1$ and $p = q$.
        ...
        \item
            ...
            \end{align*}
            Second, let $p_1, p_2 \in [0, 1]$ and $\lambda \in (0, 1)$.
	    Using basic differentiation we get $h''(p) < 0$ (omitted for brevity). That proves concavity: 
	    \[ H(X_{\lambda p_1 + (1-\lambda) p_2}) \ge \lambda H(X_{p_1}) + (1-\lambda) H(X_{p_2}) \]
    \end{enumerate}
\end{proof}
