...
\begin{remark}
    ...
    \begin{enumerate}
        ...
        \item What distribution minimises and what distribution maximises the value of Entropy? \\
            ...
            We can now write first the question as an optimisation problem: $\min_{p \in \mathbb{R}^{n}, \|p\|_1=1, p \ge 0}{H(X_p)}$. \\
            ...
            Figure \ref{fig:uniform-distr-max-entropy} shows the solver working with Gradient Descent in Tinygrad \cite{Tinygrad}.
            A similar approach can answer the second question.
            Figure \ref{fig:peak-distr-min-entropy} shows the process.
            From the data, it seems plausible that the uniform distribution maximises entropy and a peaked distribution minimises entropy.
        ...
        \item Can joint entropy increase if we add redundant information? \\
            ...
            This example suggests entropy never increases. We will prove that in the follow-up theorem.
        ...
        \item What happens to the Entropy if we add independent noise to our measurements? \\
            ...
            % TODO: Maybe add a calculation. Cannot be bothered right now, it's 2am.
...
\begin{theorem} \label{theorem:more-entropy-observations}
    ...
    \begin{enumerate}
        ...
        \item A peaked distribution minimises entropy: \\
            $H(X) \ge 0$ and $H(X) = 0 \iff \exists x \in \mathcal{X}: p_X(x) = 1$.
...
\begin{proof}
    \begin{enumerate}
        ...
        \item
            ...
            \end{align*}
            Similarly, $H(Z \mid Y) = H(X \mid Y)$. \\
            Using indep. of $X, Y$ and information never hurts, we get: \\
            $H(X) = H(X \mid Y) = H(Z \mid Y) \le H(Z)$. \\
            Similarly, we get $H(Y) = H(Y \mid X) = H(Z \mid X) \le H(Z)$.

        \item
            Using the Chain rule, we have
            \begin{align*}
                H(X_1, \cdots, X_n) &= \sum_{i=1}^{n}{H(X_i \mid X_{i-1}, \cdots, X_1)}         && \text{(chain rule)} \\
