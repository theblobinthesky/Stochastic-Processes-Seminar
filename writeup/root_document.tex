...
\begin{definition}\label{def:entropy-mutual-info-def}
    ...
    \intertext{\noindent Let $p$ and $q$ be two probability mass functions on the same set $\mathcal{Z}$. We define the following:}
        \textbf{Relative Entropy:}    & \quad D(p \| q) = \mathbb{E}_{X \sim p}\paren{\log \frac{p(X)}{q(X)}} 
            \quad \text{with conventions from Remark \ref{remark:relative-entropy-conventions}}
...
\begin{definition}\label{def:entropy-mutual-info-def}
    \begin{align*}
    \intertext{\noindent Let $p$ and $q$ be a joint probability mass function on $\mathcal{X} \times \mathcal{Y}$.}
        % TODO:
        \textbf{Conditional Relative Entropy:}    & \quad D(p(x \mid y) \| q(x \mid y)) = \mathbb{E}_{(X, Y) \sim p(x, y)}\paren{\log \frac{p(X \mid Y)}{q(X \mid Y)}} \\
        % TODO:
        \textbf{Conditional Mutual Information:}  & \quad I(X; Y \mid Z) = H(X \mid Z) - H(X \mid Y, Z)
    \end{align*}
\end{definition}

\begin{example}
    To understand the concept, we can now calculate the conditional relative entropies for an example.
\end{example}
...
\begin{corollary}
    The following specialization of the Chain Rules for Entropy and Mutual Information is commonly used:
    \begin{enumerate}
...
\begin{proof}
    \begin{enumerate}
        \item
            Let $n \in \mathbb{N} \\ \{1\}$ and $p_1, \cdots, p_n \in (0, 1)$ such that $\sum_{i=1}^{n}{p_i} = 1$ and $x_1, \cdots, x_n \in \mathbb{R}$.
