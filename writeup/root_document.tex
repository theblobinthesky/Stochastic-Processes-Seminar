...
\begin{definition}
    Let $(\Omega, \mathcal{A}, \mathds{P})$ be a probability space, $\mathcal{X}, \mathcal{Y}$ be countable sets and
    $X: \Omega \rightarrow \mathcal{X}$, $Y: \Omega \rightarrow \mathcal{Y}$ be discrete random variables on $(\Omega, \mathcal{A}, \mathds{P})$. \\
    We can then define
    \begin{align*}
        \text{Entropy wrt. base:}       &\quad H_q(X) = \mathbb{E}\paren{-\log_q p(X)} = -\sum_{x \in \supp(X)}{p(x) \log_q p(x)} \\
        \text{Entropy conventionally:}  &\quad H(X) = H_2(X)
    \end{align*}
\end{definition}
...
\begin{remark}
    \begin{enumerate}
    \item
        We use the shorthand notations $p(x) = \mathds{P}[X = x]$ and $p(y) = \mathds{P}[Y = y]$.
        If the context is unclear, the normal notations will be used to avoid confusion!
...
\begin{example}
    ...
    An increase in $p$, so less variance and concentration towards zero, seems to be linked to a lower entropy and vice-versa.
...
\begin{definition}\label{def:entropy-mutual-info-def}
    ...
    \begin{align*}
        ...
        \text{Relative Entropy:}    & \quad D(p(x) \| q(x)) = \mathbb{E}_p\paren{\log \frac{p(X)}{q(X)}} \\
                                    & \quad \text{with the conventions} 0 \log 0 = 0,   \\
        \text{Mutual information:}  & \quad I(X; Y) = D(p(x, y) \| p(x)p(y)) \\
        \text{Conditional Mutual information:}  & \quad I(X; Y \mid Z) = H(X \mid Z) - H(X \mid Y, Z)
    \end{align*}
\end{definition}
...
\begin{remark}
    ...
    We can now calculate the relative entropies for an example.
...
\begin{theorem}\label{theorem:chain-rules}
    Let $n \in \mathbb{N}, n \ge 2$, $(X_1, \cdots, X_n) \sim p(x_1, \cdots, x_n)$.
...
\begin{proof}
    \begin{enumerate}
        \item Prove this result using induction by $n$. \\
        Base case $n = 2$:
        \begin{align*}
...
\begin{corollary}
    We can specialize the Chain Rule for Entropy and Mutual Information to two variables:
    \begin{enumerate}
